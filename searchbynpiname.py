...
def checknpiname(pname, inbar):
    myresult = []
    headers = {'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.142 Safari/537.36',
           'x-requested-with': 'XMLHttpRequest'}
    url = 'https://npiprofile.com/search-advanced.php?sProvider_Business_Name='
    if len(inbar) < 2 and len(pname) < 2:
        return json.dumps({'status':'Error'})
    myquery = ''
    if len(pname) > 2:
        myquery = str(pname)
        query=str(pname).replace(" ","%20").replace("'S","").replace("'s","").split("(")[0].strip()
    ...
    for div in divlist:   
        companylink = 'https://npiprofile.com/npi/' + str(div.get_text().strip())
        print(companylink)
        ...
        page = requests.post(companylink, headers=headers)
        soup = BeautifulSoup(page.text,'html.parser')
        ...
        try:
            ...
            pass
        if (str(inbar.strip()) in str(pmail_barcode).strip()) or (str(pmail_barcode.strip()) in str(inbar.strip())) or (str(inbar.strip()) in str(ploc_barcode).strip()) or (str(ploc_barcode.strip()) in str(inbar.strip())):
            myresult.append({'provider_npi':str(npi),'provider_name':provider_name,'provider_loc_add':ploc,'provider_mail_add':pmail,'npi_entity_type':npientity,
