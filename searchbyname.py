...
def checkboth(inbar,name):
    ...
    if len(inputstr) > 2:
        ...
        divlist = soup.findAll('div',{'class':'panel-body'})
        if len(divlist) == 0:
    ...
    for div in divlist:
        ...
        try:
            ...
            pass
        if (str(inbar.strip()) in str(barcode).strip()) or (str(barcode.strip()) in str(inbar.strip())) or mycheck == 1:
